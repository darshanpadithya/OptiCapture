...
normal_output_folder = 'NormalVideo/'  # Folder for normal continuous recordings
recording = False
out = None
...
combine_time = "19:32"  # Time to combine videos (HH:MM format)
normal_record_end_time = "23:59"  # Time to stop normal continuous recording (HH:MM format)
...
# Start normal continuous recording
timestamp = time.strftime("%Y%m%d-%H%M%S")
normal_output_file = normal_output_folder + f'normal_video_{timestamp}.avi'
fourcc = cv2.VideoWriter_fourcc(*'XVID')
normal_out = cv2.VideoWriter(normal_output_file, fourcc, 20.0, (640, 480))  # Adjust resolution as needed
...
while True:
    ...
    # Write the normal continuous recording
    ...
    # Check if it's time to stop normal continuous recording
    if current_hour_minute == normal_record_end_time:
        print(f"Stopping normal continuous recording at {normal_record_end_time}...")
        normal_out.release()
        break
